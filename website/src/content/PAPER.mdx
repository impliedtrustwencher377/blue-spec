...
**Blue Spec** is the reference implementation. It realizes SDH across 36 AI coding agents and proves the methodology is buildable and portable. This is experimental work, run only in controlled environments ahead of public release, offered as a convention others can adopt.
...
- A dedicated comparison with Spec-Driven Development, the lineage SDH grew from and then diverged from (`§IX`).
- The engineering disciplines that make the methodology hold under refactors and across agents: identity-by-name tracking (`§X`) and an agent-agnostic carrier (`§XI`).
...
These principles are not ornamental. They are the constraints every phase below must respect, and the discussion in `§XIV` returns to them to mark the methodology's limits.

The rest of the document develops eight named pillars. They are the parts a reader should be able to keep in mind, and the parts another team would reimplement to claim they are doing SDH.
...
| Identity-by-name tracking                           | A finding stays one item across phases, and a rename breaks the link in one place.               | `§X`        |
| The agent-agnostic carrier                          | The security value is written once and runs across agents and stacks.                            | `§XI`       |

**Table 2.** The eight named pillars of SDH and where each is developed.

---

## IV. The 5-phase Blue Team flow

SDH structures the work as a five-phase defensive lifecycle (Table 3). Each phase is framed around defense, each builds on the previous, and the phases follow real Blue Team activity: establish policy, detect, triage, remediate, verify.

| #   | Phase   | Blue Team verb | Reads                                     | Writes                                                | Key invariant                                                                         |
| --- | ------- | -------------- | ----------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------- |
| 1   | charter | policy         | the project, or the user's description    | the charter                                           | Principles carry the risk they prevent, in plain language.                            |
| 2   | detect  | detect         | the user's code                           | the detect map                                        | Detection, not invention. Record only what the code supports.                         |
| 3   | plan    | triage         | the detect map                            | the defense plan                                      | Plan never reads code. It works only from what detect mapped.                         |
| 4   | harden  | remediate      | the defense plan, and the code it touches | the code, and the hardening record                    | The only phase that changes code. One fix at a time, confirmed first.                 |
| 5   | verify  | verify         | the code, and the hardening record        | the verification report, and closure across the chain | Verification is the confrontation of code against record, statically. Never softened. |

**Table 3.** The five phases: what each reads, what it writes, and its load-bearing invariant.
...
>
> The same finding name carries through all four artifacts. That shared name is its identity (`§X`).

---

## V. Closability: a finding has an end state

Most security tooling leaves the user with a backlog that only grows. SDH adopts a different position: **a finding is closable, so it has an end state instead of living in an endless backlog** (Figure 1). This follows directly from the reconcile principle.
...
SDH artifacts are living documents, not logs. Every time a phase re-runs, it re-checks each existing entry against the current truth and does one of three things: keeps what still holds, rewrites what changed, or removes what no longer applies. A finding the code shows is now resolved is removed, not archived for history. The record reflects the present, not the past.

Closure is that same reconcile reaching its conclusion. When verify verifies a risk closed, it stands the finding down across the whole flow, detect, plan, harden, verify, and the tracking map, with the user's confirmation. This is why verify is the one phase that writes to the other phases' artifacts: it is the phase that closes the loop. A finding stood down is simply absent everywhere afterward, and that absence is expected, not a broken chain.
...
stateDiagram-v2
    ...
    Verify --> StoodDown: risk closed + user confirm
    StoodDown --> SETTLED
...
**Figure 1.** The closability state machine. A finding moves OPEN → STOOD DOWN → SETTLED, and reopens only when the code changes.

When everything in scope is stood down, the cycle is settled. It is reopened only when the code changes. The methodology gives each finding a definite end, which is exactly what an endless backlog cannot.
...
|         | Automatic mode (run with no input)                            | Manual power (run with input)                                          |
| ------- | ------------------------------------------------------------- | ---------------------------------------------------------------------- |
| charter | SDH proposes principles by reading the project itself.        | The user describes the project, or hands SDH their own rules.          |
| detect  | A full scan, after warning about token cost and asking first. | Point detect at exact paths, or describe a single concern to scan for. |
| plan    | Plan a fix for every finding detect mapped.                   | Plan only the findings touching named paths, or a described worry.     |
| harden  | Apply every fix, highest priority first.                      | Apply a single fix, named files, or a chosen priority band.            |
| verify  | Verify every applied control, highest priority first.         | Verify named controls, named paths, or a chosen priority band.         |

**Table 4.** The same five phases serving the novice (automatic mode) and the expert (manual power).
...
A knowledge module is focused, language-agnostic security knowledge for one risk area. It audits and explains. It never rewrites the user's code, and it never produces an attack input. Each module names the safer shapes for the risks it covers and tells the agent how to act on a finding in detect (detection) versus verify (verification). The modules are evolutionary by design: the collection grows and reshapes over time, while the pattern around it stays the same. This section describes that pattern, not any particular module.

The defining trait is that a module is **non-invocable on its own**. Nothing opens it directly, not the user, not a phase. There is a single dispatcher entry point. A phase that needs knowledge lists the catalog, matches the project's context against what each entry covers, and loads only the modules whose area is present in scope. A module whose context is absent is skipped in silence, not announced. Finding none applicable is a valid outcome.

The catalog is data, and growth is one knowledge file plus one catalog row, never a new command. This is the same context-aware principle as the phases, applied to knowledge: the relevant knowledge is pulled in for what the project actually is, instead of every check running by default.
...
Determinism appears precisely where it matters. A module can carry a deterministic checker, not just prose. Where a risk class has a crisp decision procedure, the module ships a small program that returns a fixed verdict for a given input, for example a short set of words such as `safe`, `unsafe`, or `invalid input`. Some risk classes are deterministically decidable in exactly this way, with a body of prior work behind the decision procedure `[9]`, `[10]`, `[11]`. The agent decides which candidates are worth checking. The checker decides whether each one is dangerous. This is where SDH grounds its agent reasoning in something repeatable, without pretending the whole flow is deterministic.

Finally, the modules are **detachable**. Because the knowledge stands on its own, a user can run one directly in a prompt that has nothing to do with the full SDH flow, keeping security alongside ordinary development. A module with a deterministic checker can, for instance, screen each candidate produced during ordinary coding and keep only the safe ones. This detachability is part of why SDH adapts to so many situations, which is the subject of `§VIII`.

> **Practitioner's Note**
>
> **Security alongside coding:** A knowledge module is detachable. It can be invoked inside an ordinary coding prompt, with no SDH flow around it, to screen code as it is written. Security stays next to development instead of waiting for a separate pass.
...
The charter's propose-then-review mode is the clearest case: SDH proposes a safe-by-default charter by analyzing the project, presents it in plain language, and asks the user to review and adjust before anything is written. The user is never required to know what to ask for.

This adaptivity shows up as a sensitivity to the shape of the input. The same phase reads its input and chooses a mode.

| Input shape                   | How a phase responds                                                                                          |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------- |
| No input                      | The automatic mode: propose, or scan the whole scope (detect asks first, because a full scan is token-heavy). |
| One or more paths             | Focus only on those files or directories.                                                                     |
| A described concern           | Scope to that concern, for example "make sure customer data is never exposed."                                |
| A named finding from the list | Act on that single tracked item.                                                                              |
| A priority band               | For plan, harden, and verify: act only on Critical, High, Medium, or Low.                                     |

**Table 5.** How a phase reads the shape of its input and chooses a mode.

A file in the prompt, a finding from the list, a worry, a focused mission against a known weak point, or a request for a full brute-force scan: each fits the user's need, because the flow is non-authoritative by design. None of this weakens the safe-by-default baseline. Flexibility shapes how the baseline is applied, it never lowers it.

---

## IX. SDH versus Spec-Driven Development

SDH grew from the Spec-Driven Development (SDD) base and then diverged sharply. The structural inheritance is real and acknowledged: spec-first, agent-driven, a governing document, and a set of phased commands the agent runs. SDH reuses that structure deliberately. What differs is the purpose, the authority model, the assumed state, and the end condition. We treat SDD as a class, using Piskala's report `[1]` as the canonical statement of the methodology, GitHub's Spec Kit `[2]` as the canonical agent toolkit, and OpenSpec `[3]` as a second tool from the same family.
...
SDH inverts almost every axis of that contract (Table 6).
...
**Table 6.** SDD versus SDH across eight axes. Borrowed structure, inverted contract.
...
## X. Engineering rigor: the tracking discipline

The reconcile-never-append discipline (`§V`) needs a substrate that survives refactors. A finding must stay one item across five phases, and a renamed or moved file must not break the chain. SDH solves this with **identity-by-name** and **paths held only in tracking**.
...
Each detect finding is one tracked item. Its identity is its **name**, the section title, written identically in the detect map, the plan, the hardening record, and the verification report. The later phases reuse that name verbatim as they act on the same item. The tracking map (`.bluespec/tracking.json`) holds tracking and nothing else:
...
flowchart TB
    ...
    plan["plan.md"] --> name
    harden["harden.md"] --> name
    verify["verify.md"] --> name
...
**Figure 2.** The identity conveyor. One finding name flows through every artifact, while its path lives only in the tracking map, so a rename is corrected in one place.

Two acts maintain the map in the normal flow, and a third stands apart as maintenance. **Track** is additive registration: a phase reports the findings it wrote, and the engine records the new ones and follows a renamed path for the rest, classifying each as new, moved, or unchanged. It never removes anything the phase did not report. **Untrack** drops the findings verify stood down at closure. These two are what the phases call as they run. **Repair** is the maintenance act: a phase delegates to it automatically when its own reconciliation finds the tracking inconsistent (a recorded path that no longer exists), and otherwise it is run on demand. It realigns the whole map at once, reading every artifact and the current code, correcting each item's paths, and surfacing what it cannot decide alone, an orphan (a tracked item no artifact names anymore) or a renamed-candidate (an item whose paths match a different reported name).
...
The methodology applies its own defense discipline to itself. The deterministic engines ship as hooks invoked as a command-line program, and the input is passed as a single positional argument. Passing the input as a process argument keeps it inert, so a value with quotes or backticks can never inject into the command. SDH does not exempt its own plumbing from the safe-by-default principle.

---

## XI. The carrier: agent-agnostic architecture as a portability proof

For SDH to be a convention others adopt, it must not be tied to one agent or one stack. Blue Spec proves this is achievable. Its guiding rule is "adapters are data, not code."
...
flowchart TB
    ...
    dispatcher -.-> bluespec[".bluespec/<br/>(charter, phase artifacts, hooks,<br/>knowledge modules, tracking)"]
```

**Figure 3.** The agent-agnostic carrier. One core renders through a format dispatcher into each agent's packaging, with the project state as a shared sibling.
...
## XII. Worked case: SDH end to end
...
_This is an illustrative worked example of the methodology's mechanics. SDH is experimental and currently run only in controlled environments ahead of public release, so it is not a field-measured study and reports no efficacy metrics._
...
2. **detect** scans the upload route and the small admin page that lists uploaded files. It detects the upload finding (the handler trusts the client's MIME type). The admin page runs in the browser, a context that prompt-on-demand (`§VII`) recognizes, so detect loads the knowledge module for that area and surfaces a second finding, a place where a filename is written into the page as markup rather than text, so a crafted name can run as script in the visitor's browser. Knowledge modules for areas not present in this scope never load and are never mentioned.
...
4. **harden** applies the upload fix first (highest priority), shows the change, asks, and records it Applied. It applies the second fix next, Applied.
5. **verify** reads each spot and confronts code against record. The upload control holds: the handler detects the real type from content, enforces a size limit, and refuses the rest. Verdict: risk closed. With the user's confirmation, verify stands the upload finding down across the whole chain.
...
## XIII. When to use SDH, and its limits
...
**Figure 4.** When to use SDH. The branch is security relevance first, then depth of assurance, never project size.
...
## XIV. Discussion: positioning, limits, and what SDH does not claim

SDH is experimental, and it is precise about its boundaries.
...
Most of SDH is agent-reasoned, which is what enables its context-awareness. Determinism is reserved for the deterministic engines: the checkers a knowledge module may carry (`§VII`) and the tracking engine (`§X`). The document does not blur this line. Where a result is deterministic, it is a program's result. Where it is reasoned, it is the agent's reading of the code, with the user as the final arbiter.

### Cost and scale

A full scan is token-heavy, which is why detect warns and asks before scanning a whole project. The manual modes (`§VI`, `§VIII`) exist partly to keep the cost proportional to the concern.

### The user is the final arbiter

SDH confirms before it changes code, confirms before it stands a finding down, and asks the human to decide renamed-versus-resolved. The methodology proposes and verifies. It does not overrule the person.
...
**"Isn't this just SDD rebranded for security?"** No. SDH borrows SDD's structure and inverts its contract: authority, state assumption, end condition, and who it serves all differ (`§IX`). Same machinery, opposite direction.

**"Doesn't an AI-reasoned method just hallucinate findings?"** The design constrains invention rather than trusting the model to behave. Detect records only what the code supports ("detection, not invention," `§IV`), verify confronts code against record and may answer "cannot tell from the code" (`§XIV`), deterministic checkers ground the decidable cases (`§VII`), and the user is the final arbiter. This does not make the agent infallible, it makes its claims checkable.

A note on positioning. SDH is defined here independently of its reference implementation. The five phases, the risk-framed charter, reconcile-never-append and its closability, prompt-on-demand knowledge, identity-by-name tracking, and the defense-only boundary are the methodology. Another team could implement them in another stack and another agent ecosystem, and the result would still be SDH. Blue Spec is its proof, not its definition.

---

## XV. Pitfalls, and how SDH structurally avoids them
...
| False confidence, passing result | A present control is assumed to close the risk  | Verify confronts code against record, so "risk not closed" and "cannot tell" are first-class verdicts (`§IV`, `§XIV`) |
...
| Tooling lock-in                  | The method is welded to one vendor or agent     | Agent-agnostic carrier, adapters are data (`§XI`)                                                                     |
| The endless loop                 | The backlog never closes, the work never rests  | Closability and the settled state (`§V`)                                                                              |

**Table 7.** Known pitfalls in this methodology family and SDH's structural answer to each.
...
The methodology and its reference implementation have clear next steps, kept honest about the experimental stage.

- **Grow the knowledge catalog:**
  Each addition is one knowledge file plus one catalog row. More languages, runtimes, and risk classes are the natural direction, and the modules are expected to evolve.
...
- **Standards alignment as an optional layer:**
  Plain language is the default. A mapping from the risks SDH surfaces to established catalogs could be offered to readers who want it, without changing the user-facing experience.
- **Multi-agent breadth:**
...
AI-assisted development opened a security gap that generic checklists cannot fill, because the risk that matters is specific to what a system actually does, and the person shipping it may not know what to ask for. SDH closes that gap with a context-aware, defense-only, closable flow in which the intelligence lives in the methodology rather than the user. It detects the system's real context, triages the risks that context carries, applies fixes carefully, and verifies each one, then stands down what it has closed so each finding reaches an end state. It adapts to any development state in any stack, and it extends the developer's knowledge instead of replacing the people and tools that already do that job.

Blue Spec is the proof, but the contribution is the convention. SDH is offered as the essential security flow for the era of development with AI, for others to adopt, refine, and implement in their own stacks.

...
<!--
  ...
  "Upholds" links the fix to the charter principle it supports (.bluespec/memory/charter.md), or several separated by commas when one fix supports more than one. Write "None directly" when there is no charter or no principle fits, and keep the fix. "Priority" comes from the risk detect recorded for the finding: Critical, High, Medium, or Low. When one finding needs more than one fix, write more than one paragraph under its single block, never a second block: one finding is one item.
